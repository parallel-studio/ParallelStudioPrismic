...
export const MegaHeroItem: FC<MegaHeroItemProps<HTMLElement>> = ({
    ...
    const { register, setOptions } = useDim();

    useEffect(() => {
        if (showMobileVersion || tabletActive) {
            return;
        }

        setOptions({ blankOnHoverOut: true });

        register({
            type: "add_item",
            element: itemContainer,
        });
    }, [register, autoPlayProp, showMobileVersion, tabletActive]);
    ...
    return (
        ...
        >
            ...
            <div
                className={clsx(styles.thumbnail_container, "dim")}
                style={{ aspectRatio }}
            >
