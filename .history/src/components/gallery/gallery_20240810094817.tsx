...
export const Gallery: FC<GalleryProps<ElementType>> = ({
    ...
    useEffect(() => {
        ...
        if (container && smartImages && layoutType === "smart") {
            ...
            imgNodes.forEach((imgNode, index) => {
                const gutterCorrection = `${imgNodes.length - 1} * var(--px-small) / ${imgNodes.length}`;
                const width = `calc(${smartImages[index].width * factor} * ${gutterCorrection} px)`;
                const height = `calc(${smartImages[index].height * factor} * ${gutterCorrection} px)`;
    ...
    }, [smartImages, layoutType, containerRef, children]);
