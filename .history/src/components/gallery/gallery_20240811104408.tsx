...
export const Gallery: FC<GalleryProps<ElementType>> = ({
    ...
    useLayoutEffect(() => {
        ...
        if (width && container && smartImages && layoutType === "smart") {
            ...
            Array.from(imgNodes).forEach((imgNode, index) => {
                ...
                const width = `${smartImages.items[index].width * factor - gutterCorrection}px`;
                const height = `${smartImages.items[index].height * factor - gutterCorrection}px`;
    ...
    }, [smartImages, layoutType, containerRef, width]);
