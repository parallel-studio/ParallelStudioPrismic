...
export const useMegaHeroNew = ({ container }: UseMegaHeroProps) => {
    ...
    const autoScrollStarts = useCallback(() => {
        ...
        if (carousel && carouselItems) {
            ...
            // const currentX = gsap.getProperty(carouselItems[0], "x") as number;

            // gsap.to(carouselItems, {
            //     translateX: currentX - 150,
            ...
            // Fetch the current X position
            const currentX = gsap.getProperty(carouselItems[0], "x") as number;
            const targetX = currentX - 150; // Calculate the target X position

            gsap.fromTo(
                carouselItems,
                { translateX: currentX }, // Start from the current X position
                {
                    translateX: targetX, // Move to the target X position
                    duration: totalScrollLength / 100,
                    ease: "none",
                    onUpdate: () => {
                        const matchResult = gsap.getProperty(
