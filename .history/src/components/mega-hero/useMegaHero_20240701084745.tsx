    ...
    useEffect(() => {
        ...
        if (carousel) {
            ...
            const applyMomentum = (speed: number, element: HTMLElement) => {
                let currentSpeed = speed * 25;
                const decelerationRate = 0.92;

                const decelerate = () => {
                    element.scrollLeft += currentSpeed; // Adjust multiplier to control scroll speed
                    currentSpeed *= decelerationRate; // Deceleration rate

                    if (Math.abs(currentSpeed) > 0.1) {
                        requestAnimationFrame(decelerate); // Continue scrolling
                    } else {
