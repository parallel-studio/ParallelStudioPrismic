...
export const MegaHeroItemClientGsap: FC<MegaHeroItemProps> = ({
    ...
    const calculateSizes = useCallback(({delta, options = {direction: -1, sensivity: 0.5, duration: 0.5}}:{delta: number, options?: {direction?: number, sensivity?: number, duration?: number}}) => {
    ...
    if (!condition) return;
        ...
        const direction = e.isDragging ? 1 : -1;
            const sensitivity = e.isDragging ? 0.3 : 0.5;
            const duration = e.isDragging ? 0.5 : 0.5;
            if (e.deltaY) {
                x += e.deltaY * sensitivity * direction; // Adjust scroll speed
    ...
    useEffect(() => {
        ...
        const onBodyWheel: Observer.ObserverCallback = (e) => {
            ...
            if (e.deltaX) {
                x += e.deltaX * sensitivity * direction; // Adjust scroll speed for horizontal movement
                x = Math.max(Math.min(x, 0), offset); // Prevent scrolling beyond limits
                gsap.to(element, {
                    x: () => x,
                    duration,
