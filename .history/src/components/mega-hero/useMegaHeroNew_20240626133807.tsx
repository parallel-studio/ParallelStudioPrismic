...
export const useMegaHeroNew = ({ container }: UseMegaHeroProps) => {
    ...
    const autoScrollStops = useCallback(() => {
        const carousel = container.current;
        if (carousel && carouselItems) {
            const currentX = gsap.getProperty(carouselItems[0], "transform");
            if (!currentX) return;
            const matchResult = currentX.match(/matrix\((.+)\)/);
            const xValue = matchResult
                ? parseFloat(matchResult[1].split(",")[4])
                : 0;
