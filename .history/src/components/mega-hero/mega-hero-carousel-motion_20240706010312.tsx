...
export const MegaHeroCarouselMotion: FC<MegaHeroCarouselProps> = ({
    ...
    const x = useSlide(scrollYProgress, 1000);

    useIsomorphicLayoutEffect(() => {
        console.log(scope);
        if (scope?.current) {
            console.log("x", x);
            console.log("FIRST ELEMENT", scope.current.firstElementChild);
            animate(scope.current, { x: x }, { ease: "linear" });
        }
    }, [scope, animate, x]);

    useEffect(() => {
        console.log("yprogresse", scrollYProgress);
    }, [scrollYProgress]);

    scroll((progress) => console.log("SCROLLPROGRESS", progress));
