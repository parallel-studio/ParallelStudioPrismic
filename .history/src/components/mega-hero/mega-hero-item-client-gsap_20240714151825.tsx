...
export const MegaHeroItemClientGsap: FC<MegaHeroItemProps> = ({
    ...
    useEffect(() => {
        ...
        const onBodyWheel: Observer.ObserverCallback = (e) => {
            ...
            const sensibility = e.isDragging ? 1 : 0.5;
            const duration = e.isDragging ? 1 : 0.5;
            if (e.deltaY) {
                x += e.deltaY * sensibility * direction; // Adjust scroll speed
            ...
            if (e.deltaX) {
                x += e.deltaX * sensibility * direction; // Adjust scroll speed for horizontal movement
                x = Math.max(Math.min(x, 0), offset); // Prevent scrolling beyond limits
                gsap.to(element, {
                    x,
                    duration,
