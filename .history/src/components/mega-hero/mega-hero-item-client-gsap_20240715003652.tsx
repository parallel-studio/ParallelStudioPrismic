...
export const MegaHeroItemClientGsap: FC<MegaHeroItemProps> = ({
    ...
    const calculateMovement = useCallback(
        ({
            ...
            x.current += delta * sensivity * direction; // Adjust scroll speed
            x.current = Math.max(Math.min(x.current, 0), offset); // Prevent scrolling beyond limits
    ...
    useEffect(() => {
        ...
        const handleResize = () => {
            ...
            setupScrollTrigger();
            // You might need to call setupScrollTrigger again or update it depending on your needs
        };
