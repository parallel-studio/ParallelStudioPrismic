...
const createGrid = (container: HTMLDivElement, columns: number, gutterSize: number): void => {
    if (!container) {
        return;
    }
    ...
    const initialRowWidth = smartValues.width;
...
export const SmartGrid: FC<SmartGridProps<ElementType>> = ({
    ...
    }, [containerRef, children, smartValues, currentLayout]);
    ...
    useEffect(() => {
        ...
        if (smartValues && smartValues.items.length > 0) {
            ...
            smartValues.items.map((img, index) => {
                const row = Math.floor(index / columns);
                const column = index % columns;
                const width =
                    row === 0
    ...
    }, [width, smartValues, columns, gutterSize, children]);
