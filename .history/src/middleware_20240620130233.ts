...
function authCheck(auth: string | null | undefined) {
    if (!auth) {
        return false;
    }

    if (auth === `Bearer ${process.env.NEXT_PUBLIC_DATOCMS_CLIENT_TOKEN}`) {
        return true;
    }
...
export function middleware(request: NextRequest) {
    ...
    return i18nRouter(request, i18nConfig, responseWithHeaders);
}
