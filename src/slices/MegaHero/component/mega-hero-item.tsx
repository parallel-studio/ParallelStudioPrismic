...
const VideoOnHover = dynamic(
  () =>
    import("@/components/video/video-on-hover").then((mod) => mod.VideoOnHover),
...
export const MegaHeroItem: FC<MegaHeroItemProps<HTMLElement>> = ({
  ...
  useEffect(() => {
    ...
    const scrollTo = ({
      ...
      const elementContainerClientWidth = controller?.container?.clientWidth;
    ...
    const handleResize = () => {
      Observer.getAll().forEach((o) => {
        if (o.vars.target === "carousel") o.kill();
      });
      setupScrollTrigger();
    };
  ...
  }, [itemContainer, showMobileVersion, controller]);
  ...
  const project = hasProjectData(projectProps) ? projectProps.data : undefined;
      ...
      >
        ...
        {bypass_video && muxData && (
          <div
            className={clsx(styles.thumbnail_container, "dim-el")}
            onClick={handleOpen}
            onKeyDown={(e) => (e.key === "Enter" ? handleOpen() : undefined)}
            aria-label="Showreel"
            style={{
              aspectRatio,
            }}
