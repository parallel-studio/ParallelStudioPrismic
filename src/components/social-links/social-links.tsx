...
import { SettingsDocumentDataSocialLinksItem } from "../../../prismicio-types";
    ...
    <div className={clsx(styles[theme], className)} {...etc}>
      ...
      <ul className={styles.links}>
        {links.map((link, index) => {
          if (isFilled.link(link.link) && isFilled.keyText(link.title))
            return (
              <li key={index}>
                <LinkComponent field={link.link}>{link.title}</LinkComponent>
              </li>
            );
        })}
      </ul>
