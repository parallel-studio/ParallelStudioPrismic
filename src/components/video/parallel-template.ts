...
export type ParalellTemplateProps = {
  colors?: {
    controlBarPlaying?: string;
    backdropColor?: string;
  };
};

export const parallelTemplate = ({ colors = {} }: ParalellTemplateProps) => {
  // const controlBarPlayingColor =
  //     colors.controlBarPlaying ?? "rgba(0, 0, 0, 0.6)";

  const controlBarPlayingColor = colors.controlBarPlaying ?? "black";
  const backdropColor = colors.backdropColor ?? "black";
  ...
  return html`<style>
      ...
      media-controller:is(:not([mediapaused])) media-control-bar,
      media-controller:is([mediapaused], :not([userinactive]))
        media-control-bar {
        background-color: ${controlBarPlayingColor};
      }
      ...
      media-controller:is([mediaisfullscreen]) ::slotted([slot="media"]) {
        background-color: black;
      }

      media-controller:is([mediapaused], :not([userinactive]))::part(
          vertical-layer
        ) {
      ...
      <template if="streamtype == 'on-demand'">
        <template if="!breakpointsm">
          <media-control-bar part="control-bar bottom">
            {{>PlayButton}} {{>TimeRange}} {{>MuteButton}} {{>CaptionsButton}}
            {{>FullscreenButton}}
          </media-control-bar>
        </template>
        ...
        <template if="breakpointsm">
          <media-control-bar part="control-bar bottom">
            {{>PlayButton}} {{>SeekBackwardButton}} {{>SeekForwardButton}}
            {{>TimeRange}}
            <template if="breakpointmd">
              <media-time-display></media-time-display>
            </template>
            {{>MuteButton}} {{>VolumeRange}} {{>CaptionsButton}}
            {{>AirplayButton}} {{>CastButton}} {{>PipButton}}
            {{>FullscreenButton}}
          </media-control-bar>
        </template>
      </template>
