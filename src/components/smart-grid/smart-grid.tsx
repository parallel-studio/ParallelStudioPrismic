...
const DATA_SMART_WIDTH_NAME = "data-smart-width";
const DATA_SMART_HEIGHT_NAME = "data-smart-height";
...
const getSmartTable = (nodes: NodeList, columns: number): SmartTable => {
  const items: SmartTable["items"] = [];

  nodes.forEach((child) => {
    const element = child as HTMLElement;
    if (element) {
      const dataWidth = element?.getAttribute(DATA_SMART_WIDTH_NAME);
      const width = dataWidth ? parseInt(dataWidth) : element.clientWidth;
      const dataHeight = element?.getAttribute(DATA_SMART_HEIGHT_NAME);
      const height = dataHeight ? parseInt(dataHeight) : element.clientHeight;
    ...
    const x =
      column === 0
        ...
        : table
            .map((item) => item.width)
            .reduce(
              (acc, item, i) => (i < column ? acc + item + gutterSize : acc),
              ORIGIN_X,
            );
    const y =
      row === 0
        ...
        : table
            ...
            .reduce(
              (acc, item, i) =>
                item[2] === column ? acc + item[0] + gutterSize : acc,
              ORIGIN_Y,
            );
...
export const SmartGrid: FC<SmartGridProps<ElementType>> = ({
  ...
  const currentLayout = useMemo(
    () =>
      breakpoints[currentBreakpoint] ?? DEFAULT_BREAKPOINTS[currentBreakpoint],
  ...
  const columns = useMemo(() => {
    const element = container ? getElement(container) : undefined;
  ...
  useEffect(() => {
    ...
    setSmartTable(getSmartTable(container.childNodes, columns));
  }, [children, columns]);
